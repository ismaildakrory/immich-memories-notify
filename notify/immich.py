"""Immich API: memories, people, assets, thumbnails, albums, tags."""
...
GENERATED_TAG_NAME = "memnotify-generated"
...
def get_random_person_photo(
    ...
    for person in shuffled_persons:
        ...
        try:
            ...
            # Filter out recent photos, already-used, and app-generated assets
            valid_assets = []
            for asset in assets:
                asset_id = asset.get("id")
...
def select_asset_with_face_preference(
    ...
    for asset in available:
        ...
        try:
            ...
            if is_generated_asset(asset_details):
                continue
...
def upload_collage_to_album(
    ...
    try:
        ...
        tag_asset_as_generated(immich_url, api_key, asset_id, logger)
        return asset_id
...
def _get_or_create_tag(immich_url: str, api_key: str) -> Optional[str]:
    """Get or create the memnotify-generated tag. Returns tag ID."""
    ...
    if resp.status_code == 200:
        for tag in resp.json():
            if tag.get("name") == GENERATED_TAG_NAME:
                return tag["id"]
    resp = requests.post(f"{immich_url}/api/tags", headers=headers, json={"name": GENERATED_TAG_NAME}, timeout=10)
    if resp.status_code in (200, 201):
        return resp.json().get("id")
    return None


def tag_asset_as_generated(immich_url: str, api_key: str, asset_id: str, logger: logging.Logger):
    """Tag an asset as generated by the app."""
    try:
        tag_id = _get_or_create_tag(immich_url, api_key)
        if not tag_id:
            return
        headers = {"Accept": "application/json", "x-api-key": api_key, "Content-Type": "application/json"}
        requests.put(f"{immich_url}/api/tags/{tag_id}/assets", headers=headers, json={"ids": [asset_id]}, timeout=10)
    except Exception as e:
        logger.debug(f"Could not tag asset {asset_id}: {e}")


def is_generated_asset(asset: dict) -> bool:
    """Check if an asset was generated by the app (has the memnotify-generated tag)."""
