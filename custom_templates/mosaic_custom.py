"""
Mosaic Template - Varied-size tiles filling the canvas
=======================================================
Custom background support: mosaic_background.png (1920x1920)
Custom overlay support: mosaic_overlay.png (transparent PNG)
Custom layout: mosaic_layout.json (photo positions, sizes, rotation)
Auto-generated guide: mosaic_template.png (shows photo positions)
"""
...
def render(images, names, width, height, faces_list=None):
    """Mosaic layout with JSON-driven positioning."""
    if faces_list is None:
        faces_list = [[] for _ in images]

    template_dir = Path(__file__).parent
    bg_path = template_dir / "mosaic_background.png"
    guide_path = template_dir / "mosaic_template.png"
    layout_path = template_dir / "mosaic_layout.json"

    # Randomly select one of 3 overlays
    overlay_num = random.randint(1, 3)
    overlay_path = template_dir / f"mosaic_overlay{overlay_num}.png"
    if not overlay_path.exists():
        overlay_path = template_dir / "mosaic_overlay.png"  # Fallback
    ...
    if layout_path.exists():
        ...
        # Default mosaic positions (varied sizes)
        padding = 8
        positions = [
            {'x': padding, 'y': padding, 'width': width // 2 - padding * 2, 'height': height // 2 - padding * 2, 'rotation': 0, 'z_order': 0},
            {'x': width // 2 + padding, 'y': padding, 'width': width // 2 - padding * 2, 'height': height // 3 - padding * 2, 'rotation': 0, 'z_order': 1},
            {'x': width // 2 + padding, 'y': height // 3 + padding, 'width': width // 4 - padding * 2, 'height': height // 6 - padding * 2, 'rotation': 0, 'z_order': 2},
            {'x': width // 2 + width // 4 + padding, 'y': height // 3 + padding, 'width': width // 4 - padding * 2, 'height': height // 6 - padding * 2, 'rotation': 0, 'z_order': 3},
            {'x': width // 2 + padding, 'y': height // 2 + padding, 'width': width // 2 - padding * 2, 'height': height // 2 - padding * 2, 'rotation': 0, 'z_order': 4}
        ]
    ...
    if not guide_path.exists():
        ...
        draw.text((20, 20), "Mosaic Template Guide - Varied Sizes", fill='yellow', font=font)
